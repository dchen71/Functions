 ...
 * @version 2.0
 */
public class BinarySearchTreeWithDups_Complete<T extends Comparable<? super T>> extends
		BinaryTree<T> implements SearchTreeInterface<T>, java.io.Serializable {

	public BinarySearchTreeWithDups_Complete() {
		super();
	}

	public BinarySearchTreeWithDups_Complete(T rootEntry) {
		super();
        ...
        if(currentNode != null)
        {
            ...
            if (searchVal.equals(currentEntry))
            {
                ...
                if(currentNode.getLeftChild() != null)
                    lResult = supGetEnt(currentNode.getLeftChild(), searchVal);
                if(currentNode.getRightChild() != null)
                    rResult = supGetEnt(currentNode.getRightChild(), searchVal);

                if(lResult != 0)
                {
                    for(int start = 0; start < lResult; start++)
                    {
                        result.add(searchVal);
                    }
                }

                if(rResult != 0)
                {
                    for(int start = 0; start < rResult; start++)
                    {
                        result.add(searchVal);
                    }
                }
    ...
    private int supGetEnt(BinaryNodeInterface node, T searchVal)
    {
        int result = 0;
		BinaryNodeInterface<T> currentNode = node;
		...
		if (currentNode != null)
        {
            ...
            if (searchVal.equals(currentEntry))
            {
                result += 1;
                if(currentNode.getLeftChild() != null)
                    result += supGetEnt(currentNode.getLeftChild(), searchVal);

                if(currentNode.getRightChild() != null)
                    result += supGetEnt(currentNode.getRightChild(), searchVal);
            }
            else
            {
                if(currentNode.getLeftChild() != null)
